...
let _myWorkOrdersPeriod = 'today';
let _myWorkOrdersRaw = [];
let _myWorkOrdersLinkedIds = new Set();

function _myWorkOrdersRange(period) {
  const now = new Date();
  const todayStr2 = now.toISOString().slice(0,10);
  if (period === 'today') return { from: todayStr2, to: todayStr2 };
  if (period === 'week') {
...
function myWorkOrdersSelectPeriod(period) {
  _myWorkOrdersPeriod = period;
  _renderMyWorkOrders();
}
window.myWorkOrdersSelectPeriod = myWorkOrdersSelectPeriod;

async function showMyWorkOrders(){
  showModal('modal-my-work-orders');
  await _verifyActiveEntryStillActive();
  _myWorkOrdersPeriod = 'today';
  const fixedHeader = document.getElementById('my-work-orders-fixed-header');
  ...
  try{
    ...
    const allOrders = Object.values(byId).filter(o => ['pending','in_progress','pending_review','closed'].includes(o.status));
    ...
    const openIds = allOrders.filter(o => o.status!=='closed').map(o => o.id);
    ...
    _myWorkOrdersRaw = allOrders;
    _myWorkOrdersLinkedIds = myLinkedTaskIds;
    fixedHeader.innerHTML = `<div class="entries-period-bar">
      ${[['today',t('entries.periodToday')],['week',t('entries.periodWeek')],['month',t('entries.periodMonth')],['all',t('worker.periodAll')||'הכל']]
        .map(([key,label]) => `<button class="entries-period-btn ${_myWorkOrdersPeriod===key?'active':''}" onclick="myWorkOrdersSelectPeriod('${key}')">${label}</button>`).join('')}
    </div>`;
    _renderMyWorkOrders();
  }catch(e){
...
function _renderMyWorkOrders(){
  const container = document.getElementById('my-work-orders-content');
  const fixedHeader = document.getElementById('my-work-orders-fixed-header');
  // Keep the active-state highlight on the period buttons in sync without
  // re-rendering the whole bar (avoids losing focus/flicker on tap).
  fixedHeader.querySelectorAll('.entries-period-btn').forEach(btn => btn.classList.remove('active'));
  const activeBtn = [...fixedHeader.querySelectorAll('.entries-period-btn')].find(b => b.getAttribute('onclick')?.includes(`'${_myWorkOrdersPeriod}'`));
  if (activeBtn) activeBtn.classList.add('active');

  const range = _myWorkOrdersRange(_myWorkOrdersPeriod);
  const orders = _myWorkOrdersRaw.filter(o => {
    if (!range) return true; // 'all'
    if (!o.executionDate) return false;
    return o.executionDate >= range.from && o.executionDate <= range.to;
  });
  const myLinkedTaskIds = _myWorkOrdersLinkedIds;
...
window._renderMyWorkOrders = _renderMyWorkOrders;
